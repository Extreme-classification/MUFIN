...
./run_tokeniz.sh $dataset $txt_model
...
--img_model ${img_model} --data_dir ${data_dir}/${dataset} --txt_model $txt_model \
--meta_dir ${meta_data_folder} --config configs/${dset_json} \
...
mkdir -p $result_dir $model_dir ${data_dir}/${dataset}/$txt_model
...
module2() {
    ...
    extra_args="--extract_x_img test.img.${extension} --extract_x_txt ${txt_model}/tst_X_Xf.seq.memmap \
    ...
    extra_args="--extract_x_img train.img.${extension} --extract_x_txt ${txt_model}/trn_X_Xf.seq.memmap \
...
module3() {
    ...
    extra_args="--extract_x_img test.img.${extension} --extract_x_txt ${txt_model}/tst_X_Xf.seq.memmap --extract_fname module3/test"
    python -u mufin.py $PARAMS --mode extract ${extra_args} --module 3 | tee -a $log_ex_file

    extra_args="--extract_x_img train.img.${extension} --extract_x_txt ${txt_model}/trn_X_Xf.seq.memmap --extract_fname module3/train"
    python -u mufin.py $PARAMS --mode extract ${extra_args} --module 3 | tee -a $log_ex_file

    extra_args="--extract_x_img label.img.${extension} --extract_x_txt ${txt_model}/lbl_X_Xf.seq.memmap --extract_fname module3/label"
    python -u mufin.py $PARAMS --mode extract ${extra_args} --module 3 | tee -a $log_ex_file
}
...
module4pp() {
    ...
    log_rk_file="${result_dir}/log_ranker_${ranker}.txt"
    XPARAMS="${PARAMS} --model_out_name model_${ranker}.pkl --ranker ${ranker} \
    --extract_x_img test.img.${extension}  --extract_x_txt ${txt_model}/tst_X_Xf.seq.memmap \
...
module4 XAttnRanker
./run_extract.sh $model_type $dataset $version XAttnRanker 0.7 0.9
